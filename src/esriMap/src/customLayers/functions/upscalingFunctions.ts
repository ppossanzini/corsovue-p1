...
export function nearest(pixels, x, y, offset, width) {
  return pixels[offset + Math.round(y) * width * 4 + Math.round(x) * 4];
}

export function nearest_unrolled(pixels, x, y, width) {
  var yw4x4 = (Math.ceil(y + 0.5) * width + Math.ceil(x + 0.5)) * 4 ;
  return [
    pixels[yw4x4],
    pixels[yw4x4 + 1],
    pixels[yw4x4 + 2]
...
export function bilinear(pixels, x, y, offset, width) {
...
export function bilinear_optimized(pixels, x, y, offset, width) {
  var percentX = x - (x ^ 0);
...
export function bilinear_unrolled(pixels, x, y, width) {
  var percentX = x - (x ^ 0);
  var percentX1 = 1.0 - percentX;
  var percentY = y - (y ^ 0);
...
export function bicubic(pixels, x, y, offset, width) {
  var v = [0, 0, 0, 0];
  var fx = Math.floor(x);
  var fy = Math.floor(y);
  var percentX = x - fx;
...
export function bicubic_optimized(pixels, x, y, offset, width) {
...
export function bicubic_unrolled(pixels, x, y, width) {
